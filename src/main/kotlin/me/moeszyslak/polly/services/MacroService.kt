...
@Service
class MacroService(private val configuration: Configuration) {
    fun addMacro(guild: Guild, name: String, category: String, channel: TextChannel?, contents: String): String {

        val channelId = channel?.id?.longValue
        val config = configuration[guild.id.longValue] ?: return "Guild is not setup"

        return if (config.macros.containsKey("$name#$channelId")) {
            config.macros["$name#$channelId"] = Macro(name, contents, channelId, category)
            "Success. Macro `$name` is now available ${if (channel == null) "globally" else "in ${channel.mention}"} and will respond with ```\n$contents\n```"
        } else {
